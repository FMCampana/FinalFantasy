...
if(cdeath==false&&ic!=null)g.drawImage(ic.getImage(),(int)w*1/5,(int)h*7/12,null); //Draws a character at the specified location
    ...
    int input (Character self, monster a, monster b, monster c) {
        ...
        if(self.job==6)//Red Mages have 2 job abilities instead of one, and thus need a special interface
            return 1+JOptionPane.showOptionDialog(null, "Input commands for " + self.name + " (HP: " + self.tempHP + "/" + self.hp + ")", 
    ...
    void battle(Character one, Character two, 
            Character three, Character four, monster a, monster b, monster c, Party party){
        int count1 = 0;
        int count2 = 0;
        int count3 = 0;
        int count4 = 0;
        ...
        battleScreen.setBounds(200, 100, 720, 393);
        ...
        battleScreen.setIconImage(a.image.getImage());
        ...
        if(monsterDeathCheck==3)
            victory.obtainVictory(worth, one, two, three, four, party,battleScreen);
    ...
    int attack(int str, int atk){//used for characters, since characters have weapons
        attack=str+atk;
        int x = generator.nextInt(26)+100;
        double dattack=attack*x/100;
        attack=(int)dattack;
        return attack; //The power of attack is equal to the char's STR times a random value between 1 and 1.25
    }
