...
public class Character {
    ...
    String[] commandsRed = new String[4];
    ...
    void combatInput(Combat combat, monster a, monster b, monster c, Character one, Character two, Character three,
        ...
        if(command<1){//if no input was given, turn is skipped
            ...
            while(validChoice==0){
                ...
                int x=0;
            if(a.death==0){
            enemies[x]=a.name + " (HP: " + a.tempHP + "/" + a.hp +")";x++;}
            if(b.death==0){
            enemies[x]=b.name + " (HP: " + b.tempHP + "/" + b.hp +")";x++;}
            if(c.death==0){
            enemies[x]=c.name + " (HP: " + c.tempHP + "/" + c.hp +")";x++;}
            enemies[x]="Return";
            ...
            if(enemies[target].startsWith(a.name)&&a.death==0){
                if (focus==true){
                hit*=2.5;
                focus=false;}
                a.getHit(hit,true,name);
                validChoice=1;}
            else if(enemies[target].startsWith(b.name)&&b.death==0){
                if (focus==true){
                hit*=2.5;
                focus=false;}
                b.getHit(hit,true,name);
                validChoice=1;}
            else if(enemies[target].startsWith(c.name)&&c.death==0){
                if (focus==true){
                hit*=2.5;
                focus=false;}
                c.getHit(hit,true,name);
                validChoice=1;}
            else if(enemies[target].equalsIgnoreCase("Return")){validChoice=2;}
            }
        }
        //command is the input, ability is the ability
           ...
           validChoice = party.useItem(one, two, three, four, this);}
    ...
    void fire(int mag, Character caster, monster a, monster b, monster c){
        ...
        JLabel[] message = new JLabel[3];
            ...
            if(a.death==0){fire=caster.mag+generator.nextInt(5);
                a.getHit(fire,false,name);//reduce enemy hp based on fire value
                if(a.tempHP<=0)
                    message[0]=new JLabel(a.name+" took "+fire+" damage. "+a.name+" was slain!",a.image,SwingConstants.LEFT);
                else
                    message[0]=new JLabel(a.name+" took "+fire+" damage",a.image,SwingConstants.LEFT);
                message[0]=new JLabel(a.name+" took "+fire+" damage",a.image,SwingConstants.LEFT);}
            if(b.death==0){fire=caster.mag+generator.nextInt(5);
                b.getHit(fire,false,name);
                if(a.tempHP<=0)
                    message[1]=new JLabel(b.name+" took "+fire+" damage. "+b.name+" was slain!",b.image,SwingConstants.LEFT);
                else
                    message[1]=new JLabel(b.name+" took "+fire+" damage",b.image,SwingConstants.LEFT);}
            if(c.death==0){fire=caster.mag+generator.nextInt(5);
                c.getHit(fire,false,name);
                if(a.tempHP<=0)
                    message[2]=new JLabel(c.name+" took "+fire+" damage. "+c.name+" was slain!",c.image,SwingConstants.LEFT);
                else
                    message[2]=new JLabel(c.name+" took "+fire+" damage",c.image,SwingConstants.LEFT);}
            JOptionPane.showMessageDialog(null,message,caster.name+" cast Fire!",JOptionPane.DEFAULT_OPTION);
    }
    
    void cure(Character caster, Character a, Character b, Character c, Character d){
    ...
    Weapon equip(Weapon newWeapon){
        ...
        JOptionPane.showMessageDialog(null,name+" equipped the " + newWeapon.name,"Final Fantasy",JOptionPane.DEFAULT_OPTION);
    ...
    void display(){
        ...
        JOptionPane.showMessageDialog(null, "\nName " + name+"\nClass " + jobclass +
                "\nLevel " + lv + "\nExp Until Next Level: " + exptillLV + "\nWeapon " + weapon + " (+" + atk + " STR)"+
                "\nHP " + tempHP + "/" + hp + "\nStrength " + tempstr+
                "\nMagic " + mag + "\nAgility " + agi,
                "Final Fantasy", JOptionPane.DEFAULT_OPTION, image);
        System.out.println("\nName " + name + "\t Class " + jobclass);
        System.out.println("Level " + lv + "\t Exp Until Next Level: " + exptillLV + "\t Weapon " + weapon + " (+" + atk + " STR)");
        System.out.println("HP " + tempHP + "/" + hp + "\t Strength " + tempstr);
        System.out.println("Magic " + mag + "\t Agility " + agi +"\n");
    }
